...
output_prefix="test_DSS_results"
p_val=0.001
...
DMRtest_1 = callDMR(DMLtest.t1, p.threshold=p_val)
write.table(DMRtest_1, file=paste0("08_DSS_results/", output_prefix, "_", colnames(DMLfit$X)[2], "_DMRs_p", p_val, ".txt"), sep="\t", quote=FALSE)

DMRtest_2 = callDMR(DMLtest.t2, p.threshold=p_val)
write.table(DMRtest_2, file=paste0("08_DSS_results/", output_prefix, "_", colnames(DMLfit$X)[3], "_DMRs_p", p_val, ".txt"), sep="\t", quote=FALSE)

DMRtest_3 = callDMR(DMLtest.t3, p.threshold=p_val)
write.table(DMRtest_3, file=paste0("08_DSS_results/", output_prefix, "_", colnames(DMLfit$X)[4], "_DMRs_p", p_val, ".txt"), sep="\t", quote=FALSE)
