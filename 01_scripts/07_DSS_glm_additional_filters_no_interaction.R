...
DMRegions_1 <- getMeth(BSobj, DMRtest_1, type="raw", what="perRegion")
betas_1 <- cbind(DMRtest_1[,1:3], DMRegions_1)
rownames(betas_1)<-NULL
betas_1 <- betas_1 %>% mutate_all(~ifelse(is.nan(.), NA, .))
write.table(betas_1, file=paste0("08_DSS_results/", output_prefix, "_", colnames(DMLfit$X)[2], "_DMRs_p", p_val, "_minlen", min_len, "_", min_CGs, "minCGs_", merge_distance, "bpmerge_", pct_sig_CGs, "pctsigCGs_heatmapbetas.txt"), sep="\t", quote=FALSE, row.names=FALSE)

DMRegions_2 <- getMeth(BSobj, DMRtest_2, type="raw", what="perRegion")
betas_2 <- cbind(DMRtest_2[,1:3], DMRegions_2)
