...
INPUT="/project/lbernatchez/users/clven7/hugo_capelin/dss_pipeline/04_masked_bedGraphs"
...
for i in $(ls -1 "$INPUT"/*.dedup_CpG.methylKit.gz | cut -d "." -f1) 
do
    echo $(basename $i)
    gunzip -c "$INPUT"/"$(basename $i)".dedup_CpG.methylKit.gz | 
		awk -v FS="\t" -v OFS="\t" 'NR == 1 {print} NR > 1 { if (4<$5 && 101>$5) {print} } ' |
    gzip -c - > "$OUTPUT"/"$(basename $i)""_quickfiltered.methylKit.gz"
done
